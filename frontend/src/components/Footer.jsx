...
const Footer = () => {
  ...
  const linkRoutes = {
    ...
    'Weather Advisory': '/dashboard/weather',
          ...
          <div className="sa-footer-grid">
            ...
            <div className="sa-footer-brand">
              ...
              <div className="sa-footer-socials">
                {socials.map((s) => (
                  <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="sa-social-btn" aria-label={s.label}>
                    {s.icon}
                  </a>
                ))}
              </div>
            ...
            {Object.entries(links).map(([col, items]) => (
              <div key={col}>
                ...
                <ul className="sa-footer-links">
                  {items.map((item) => (
                    <li key={item}>
                      <RouterLink to={linkRoutes[item] || '/'} className="sa-footer-link" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                        {item}
                      </RouterLink>
                    </li>
                  ))}
