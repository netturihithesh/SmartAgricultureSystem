...
const ProblemSection = () => {
  ...
  const bg = isDark ? '#1e2a24' : '#F8FAF9';
  const cardBg = isDark ? 'rgba(40,40,40,0.9)' : 'rgba(255,255,255,0.7)';
  ...
  return (
    <Box sx={{ py: { xs: '48px', md: '72px' }, backgroundColor: bg }}>
      <Container maxWidth="lg" sx={{ px: '20px' }}>
        ...
        <Typography sx={{ textAlign: 'center', color: textSecondary, mb: { xs: '32px', md: '40px' }, maxWidth: '750px', mx: 'auto', fontSize: '16px', lineHeight: 1.5 }}>
          Traditional farming relies heavily on intuition and assumptions, leaving you vulnerable to these critical challenges:
        </Typography>
        <Box sx={{
          maxWidth: '900px', mx: { xs: '-20px', md: 'auto' }, px: { xs: '20px', md: 0 },
          display: { xs: 'flex', md: 'grid' }, flexDirection: { xs: 'row', md: 'unset' },
          overflowX: { xs: 'auto', md: 'visible' }, scrollSnapType: { xs: 'x mandatory', md: 'none' },
          gridTemplateColumns: { md: 'repeat(2, 1fr)' }, gap: '24px', pb: { xs: '24px', md: 0 },
          scrollbarWidth: 'thin', scrollbarColor: 'rgba(46, 125, 50, 0.4) transparent',
          ...
          {problems.map((prob, i) => (
            <Paper key={i} elevation={0} sx={{
              flexShrink: { xs: 0, md: 1 }, width: { xs: '85vw', sm: '320px', md: 'auto' },
              scrollSnapAlign: { xs: 'center', md: 'none' }, p: '24px',
              backgroundColor: cardBg, backdropFilter: 'blur(8px)',
              border: `1px solid ${isDark ? '#333' : 'rgba(255,255,255,0.5)'}`,
              borderLeft: `4px solid ${prob.color}`,
              borderRadius: '14px', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', textAlign: 'left',
              boxShadow: isDark ? '0 8px 18px rgba(0,0,0,0.3)' : '0 8px 18px rgba(0,0,0,0.06)',
              transition: 'all 0.25s ease',
              '&:hover': { transform: 'scale(1.02) translateY(-4px)', boxShadow: isDark ? '0 16px 32px rgba(0,0,0,0.4)' : '0 16px 32px rgba(0,0,0,0.08)' }
            }}>
              <Box sx={{ mb: '16px', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '48px', width: '48px', borderRadius: '50%', backgroundColor: isDark ? '#334d3d' : '#fff', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
                {prob.icon}
              </Box>
              <Typography variant="h6" sx={{ color: '#1B5E20', fontWeight: 700, mb: '8px', fontSize: '16px' }}>{prob.title}</Typography>
              <Typography sx={{ color: textSecondary, fontSize: '14px', lineHeight: 1.4 }}>{prob.desc}</Typography>
            </Paper>
          ))}
        </Box>
      </Container>
